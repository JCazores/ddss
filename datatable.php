...
{
	$aColumns = array('s.id', 's.StudentID', 's.sname', 's.balance', 's.fees', 'b.year', 's.course', 's.contact', 's.joindate', 's.Attendance', 's.GPA');
	...
	{
	$condArr[] = "b.id = '".mysqli_real_escape_string($conn,$_GET['year'])."'";
	...
	$sWhere = " WHERE b.id=s.year and s.delete_status='0' and s.balance>0 ";
	...
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.id=s.year and s.delete_status='0' and s.balance>0  ";
...
if($_GET['type']=="report")
{
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.year','s.contact','s.joindate');
	...
	$sWhere = " WHERE b.id=s.year and s.delete_status='0'  ";
	if ( isset($_GET['sSearch'])&& $_GET['sSearch'] != "" )
	{
	...
	/* Total data set length */
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.id=s.year and s.delete_status='0'   ";
	$rResultTotal = $conn->query( $sQuery ) or die(mysqli_error($conn));
	$aResultTotal = $rResultTotal->fetch_assoc();
	$iTotal = $aResultTotal['cc'];
